  ...
  readSection([section, sectionLoc], params, forceRange) {
    ...
    switch (section[0]) {
      ...
      case "play":
        ...
        return read(namedSections, section[1]).section.withRange(range);
      case "reinterpret":
        return this.readSection(
          read(namedSections, section[1]).instrs,
          params,
          range
        );
      default:
  ...
  readInstructions(
    ...
    for (const instrWithLoc of instrs.slice(1)) {
      ...
      switch (instr[0]) {
        ...
        default:
          sectionList.push(
            this.readSection(instr, { bpm, swing, namedSections }, forceRange)
          );
      }
    }
    pop(namedSections);
