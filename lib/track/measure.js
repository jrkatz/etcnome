...
const getBeats = ({ bpm, rhythms, denom }) => {
  if (!bpm > 0) {
    throw new Error("Measure requires a positive bpm");
  }
...
class Measure extends Section {
  constructor({ bpm, swing }, rhythms, denom = 4) {
    super();
    this.beats = getBeats({ bpm, rhythms, denom });
  ...
  nextBeat(prevBeat) {
    ...
    const sound = new Sound(duration, new SoundSpec(soundSpec), this, count);
    return sound;
  }
}
